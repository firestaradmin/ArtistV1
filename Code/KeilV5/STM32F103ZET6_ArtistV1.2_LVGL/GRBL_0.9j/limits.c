...
void limits_init(void) 
{
  ...
  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
	...
	if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
		...
		HW_EXTI_Enable(X_LIMIT_INT_CHANNEL,0,0);	//使能中断
		HW_EXTI_Enable(Y_LIMIT_INT_CHANNEL,1,0);	//使能中断
		HW_EXTI_Enable(Z_LIMIT_INT_CHANNEL,2,0);	//使能中断
  	} else {
    	limits_disable(); 
  	}
...
#if defined(CPU_MAP_STM32F10X)
	HW_EXTI_Disable(X_LIMIT_INT_CHANNEL,0,0);
	HW_EXTI_Disable(Y_LIMIT_INT_CHANNEL,1,0);
	HW_EXTI_Disable(Z_LIMIT_INT_CHANNEL,2,0);	
#endif		//end of CPU_MAP_STM32F10X
}
...
//	{     
//		EXTI_ClearITPendingBit(Z_LIMIT_INT_LINE); 
//		if (sys.state != STATE_ALARM) { 
//      		if (!(sys_rt_exec_alarm)) {
//        	#ifdef HARD_LIMIT_FORCE_STATE_CHECK
//          	// Check limit pin state.  
...
//	}
	...
	{     
		EXTI_ClearITPendingBit(X_LIMIT_INT_LINE); 
		if (sys.state != STATE_ALARM) { 
      		if (!(sys_rt_exec_alarm)) {
        	#ifdef HARD_LIMIT_FORCE_STATE_CHECK
          	// Check limit pin state.  
	...
	{     
		EXTI_ClearITPendingBit(Y_LIMIT_INT_LINE); 
		if (sys.state != STATE_ALARM) { 
      		if (!(sys_rt_exec_alarm)) {
        	#ifdef HARD_LIMIT_FORCE_STATE_CHECK
          	// Check limit pin state.  
...
void limits_go_home(uint8_t cycle_mask) 
{
  ...
  do {
    ...
    DelayMs(settings.homing_debounce_delay); // Delay to allow transient dynamics to dissipate.
