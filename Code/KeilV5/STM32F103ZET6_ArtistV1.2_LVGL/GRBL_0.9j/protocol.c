...
extern lv_obj_t* btn_startprint;
...
void protocol_main_loop(void)
{
  ...
  for (;;) {
    ...
    if(off_line_print_flag != 0){	//off_line
		lv_obj_t* mbox;
		u8 res;
		u8 namebuf[255];
		sprintf((char*)namebuf, "0://%s", file_name);
		res = mf_open(namebuf, FA_READ);
		if(res == 0){
//			mbox = lv_msgbox_create(lv_scr_act(), NULL);//创建消息框
//			lv_obj_set_width(mbox, 300);//设置宽度
//			lv_obj_align(mbox, lv_scr_act(), LV_ALIGN_IN_TOP_MID, 0, LV_DPI / 2);//按钮居中
//			lv_msgbox_set_text(mbox, "printing... don't touch");
		}
		else{
			lv_msg_inform("open file failed!");
			off_line_print_flag = 0;
			goto jump1;
		}
		...
		u32 readBytes = 0, t = 0;
		do{
			res=f_read(file,fatbuf,512,&readBytes);
			if(res)
			{
				printf("Read Error:%d\r\n",res);
				break;
			...
			{
				for(t=0;t<readBytes;t++)//printf("%c",fatbuf[t]); 
				{
					c = fatbuf[t];
		...
		mf_close();
//		lv_obj_del(mbox);
		lv_btn_set_state(btn_startprint, LV_BTN_STATE_CHECKED_RELEASED);
        ...
        if ((sys.state == STATE_IDLE) || ((sys.state & (STATE_HOLD | STATE_MOTION_CANCEL)) && (sys.suspend & SUSPEND_ENABLE_READY))) {
          ...
          if (sys.suspend & SUSPEND_ENERGIZE) { 
            ...
            if (gc_state.modal.spindle != SPINDLE_DISABLE) { 
              ...
              DelayMs(SAFETY_DOOR_SPINDLE_DELAY); // TODO: Blocking function call. Need a non-blocking one eventually.
            }
            if (gc_state.modal.coolant != COOLANT_DISABLE) { 
              coolant_set_state(gc_state.modal.coolant); 
              DelayMs(SAFETY_DOOR_COOLANT_DELAY); // TODO: Blocking function call. Need a non-blocking one eventually.
            }
