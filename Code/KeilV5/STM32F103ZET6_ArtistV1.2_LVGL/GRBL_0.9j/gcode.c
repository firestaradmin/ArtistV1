...
/* -------------------------------------------------------------------------------------
     ...
     block. This struct contains all of the necessary information to execute the block. 
		初始化解析器块结构并复制当前g-code状态模式。解析器
		更新这些模式和命令，因为块行是解析器，将只使用和
		在错误检查成功后执行。解析器块结构也包含一个块
		值结构，字跟踪变量，和一个非模态命令跟踪新
		块。这个结构体包含执行块所需的所有信息*/
...
/* -------------------------------------------------------------------------------------
	 ...
	 导入块行的所有g码。g码字是后跟一个字母
	 一个数字，可以是“G”/“M”命令或设置/分配命令值。同时,
	 对于任何重复的命令字模态组违反，执行初始错误检查
	 对于负的值F N P T和S。*/
  ...
  */ 
  ...
  if (gc_state.modal.motion != MOTION_MODE_NONE) {
    if (axis_command == AXIS_COMMAND_MOTION_MODE) {
	  ...
	  lv_updata_axis_label(gc_state.position);
    }
  }
